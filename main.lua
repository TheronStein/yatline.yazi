--- @since 25.5.31
--- @updated 25.12.09 - Refactored for Yazi nightly API compatibility
---   - cx.tasks.progress -> cx.tasks.summary
---   - tasks.succ/fail -> tasks.success/failed
---   - tasks.found/processed -> tasks.percent (percentage-based)
---   - cwd.frag -> cwd.domain
---   - ya.round -> math.floor
---   - Added Tabs component support in Root.layout/build
...
function Yatline.coloreds.get:task_workload()
	...
	local percent = tasks.percent or 0
	local remaining = tasks.total - tasks.success

	local coloreds = {
		{ string.format(" %s %d%% ", task_found_icon, math.floor(percent)), task_found_fg },
		{ string.format(" %s %d ", task_processed_icon, remaining), task_processed_fg },
	}
...
return {
	setup = function(_, config, pre_theme)
		...
		Progress.redraw = function(self)
			...
			local left = summary.total - summary.success
			return gauge
				:percent(percent)
				:label(ui.Span(string.format("%3d%%, %d left", math.floor(percent), left)):style(th.status.progress_label))
		end
		...
		Root.layout = function(self)
			...
			for _, component in ipairs(component_positions) do
				...
				elseif component == "tabs" then
					table.insert(constraints, ui.Constraint.Length(Tabs.height()))
		...
		Root.build = function(self)
			...
			for _, component in ipairs(component_positions) do
				if component == "header" and display_header_line then
					...
					i = i + 1
				elseif component == "tabs" then
					table.insert(childrens, Tabs:new(self._chunks[i]))
					i = i + 1
				elseif component == "tab" then
